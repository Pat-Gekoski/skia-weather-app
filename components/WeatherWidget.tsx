...
import { Canvas, FitBox, LinearGradient, Path, rect, Skia, usePathInterpolation, vec } from '@shopify/react-native-skia'
...
const WeatherWidget = ({ forecast }: WeatherWidgetProps) => {
	...
	return (
		<Pressable style={{ height: widgetHeight, width: widgetWidth }} onPress={() => onPress()}>
			<Canvas style={{ ...StyleSheet.absoluteFillObject }}>
				<FitBox src={rect(0, 0, 342, 175)} dst={rect(0, 0, widgetWidth, widgetHeight)}>
					<Path path={path}>
						<LinearGradient
			...
			<View style={{ flex: 1, paddingTop: 40, paddingLeft: 20 }}>
				...
				<Animated.Image
					source={icon}
					style={[
						{ ...StyleSheet.absoluteFillObject, width: 160, height: 160, left: widgetWidth - 160, top: -30 },
						animatedImageStyles,
					]}
				/>
				<View style={{ paddingTop: 10 }}>
					...
					<View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingRight: 24 }}>
